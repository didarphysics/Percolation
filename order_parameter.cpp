// this is the main code for simulating susceptibility of L*L latice using ziff-newman algorithm
...
void order_parameter(int en,int l,int big){
  largest_cluster[l][en]=big;
}
...
void percolate()
 {
  for(int en=0;en<ensemble;en++){
    ...
    //if(en%100==0){ cout<<"ensemble "<<en+1<<endl; }
...
void avg_largest_cluster(int v){
  string name2="L_"+conversion_int_to_string(L)+"/largest_cluster_L" + conversion_int_to_string(L) +"_E_"+ conversion_int_to_string(ensemble) +"_V_" + conversion_int_to_string(v) +".txt";
  ofstream sp2(name2.c_str());

  for(int l=0;l<N;l++){
    double sum_largest_cluster=0.0;

    for(int en=0;en<ensemble;en++){
     // sum_largest_cluster=accumulate(largest_cluster[l].begin(),largest_cluster[l].end(),0.0);
      sum_largest_cluster+=largest_cluster[l][en];

    }
    sp2.precision(10);
    sp2<<fixed<<(double)(l+1)/N<<"\t\t"<<sum_largest_cluster/(N*ensemble)<<endl;
  }
  sp2.close();
}

void avg_entropy(int v){
...
void combine_largest_cluster(){
  ...
  for(int i=0;i<newData_C.size();i++){
    ...
    avg_C[i] /=(double)(version_to);
  }
}

void Jump_workshop(){
  string name2= "res/largest_cluster_L" + conversion_int_to_string(L) +"_E_"+ conversion_int_to_string(ensemble*(version_to+1)) +".txt";
...
void canonical_ensemble(){
  std::string name1 = "res/entropy_L_" + conversion_int_to_string(L)+"_E"+conversion_int_to_string(ensemble*version_to) +".txt";
     ...
     // entropy.clear();
      ...
      for (int in=0;in<N;in++){
        ...
        // entropy.push_back(empty1);
         ...
         largest_cluster.push_back(empty1);

         for(int en=0;en<ensemble;en++) {

           //entropy[in].push_back(0.0);
           //disequilibrium[in].push_back(0.0);
           largest_cluster[in].push_back(0.0);
          }
        }

    percolate();
    //avg_entropy(v);
    avg_largest_cluster(v);
    }
  //combine_entropy();
  //canonical_ensemble();
    combine_largest_cluster();
    Jump_workshop();
  return 0;
}
